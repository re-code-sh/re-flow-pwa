...
import { Trash2, ArrowUpRight, Search, Plus, Brain as BrainIcon } from 'lucide-react';
import { db } from '../db';
...
export const BrainDump: React.FC<BrainDumpProps> = ({ isOpen, onClose }) => {
  ...
  const handleSaveThought = async () => {
    ...
    try {
      await repo.addThought(inputVal.trim(), category);
      setInputVal('');
        ...
        <div className="space-y-2 max-h-[40vh] overflow-y-auto custom-scrollbar pr-1">
          {filteredThoughts.length > 0 ? (
            ...
            <div className="py-8 text-center text-ink3 text-xs space-y-2">
              <BrainIcon className="w-8 h-8 mx-auto opacity-30 text-[var(--accent)]" />
